      ...
      <View style={styles.modalOverlay}>
        <ScrollView contentContainerStyle={styles.modalCard}>
          ...
          {mode==='Custom' && <>
            ...
            <Button
              title={custom.start.toISOString().slice(0,10)}
              onPress={()=>setShowDP('start')}
            />
            <Text style={styles.modalLabel}>End</Text>
            <Button
              title={custom.end.toISOString().slice(0,10)}
              onPress={()=>setShowDP('end')}
            />
            {showDP && (
              <DateTimePicker
                value={custom[showDP]}
          ...
          <View key={g.location} style={styles.card}>
            ...
            {g.leaderboard.length>0
              ? g.leaderboard.map((d,i)=>(
                  <View key={d.doctor} style={styles.lbRow}>
        ...
        <ScrollView style={{flex:1}}>
          {kpiType === 'location'
            ...
            : data.byDoctor.map(group => {
                return (
                  <View key={group.location} style={styles.card}>
                    <Text style={styles.cardTitle}>By Dr — {group.location}</Text>
                    {group.perDoctor.map(d => {
